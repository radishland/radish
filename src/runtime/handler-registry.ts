...
export class HandlerRegistry extends HTMLElement
  ...
  /**
   * A reference to an `HandlerRegistry`'s `AbortController`.
   *
   * The `abort` method of the controller is called in the `disconnectedCallback` method of the handler. It allows to cleanup event handlers and other abortable operations
   */
  ...
  /**
   * Create an effect that is automatically cleaned up when the component is disconnected
   *
   * An optional AbortSignal can be provided to abort the effect prematurely
   */
  $effect(cb: EffectCallback, options?: EffectOptions) {
          ...
          if (classList && typeof classList === "object") {
            for (const [k, v] of Object.entries(classList)) {
              ...
              for (const className of k.split(" ")) {
                (e.target as HTMLElement).classList.toggle(className, force);
              }
            }
          }
        });
